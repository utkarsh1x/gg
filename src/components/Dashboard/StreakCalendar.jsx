  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col justify-between">
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-6">
          {weekDays.map(day => (
            <button
              ...
              className={`p-1 sm:p-2.5 rounded-lg sm:rounded-xl border flex flex-col items-center justify-between transition-all duration-200 min-w-0 ${
                day.isCompleted
            ...
            >
              <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider w-full text-center truncate">{day.name}</span>
              <span className="text-xs sm:text-sm font-black my-0.5 sm:my-1">{day.dateNum}</span>
      ...
      <div>
        ...
        <div className="grid grid-cols-[repeat(14,minmax(0,1fr))] gap-1 sm:gap-1.5 p-3 rounded-xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 max-w-xl mx-auto sm:mx-0">
          {heatmapDays.map(d => (
            <div
              key={d.dateStr}
