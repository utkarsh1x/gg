      ...
      {filteredSyllabus.map(phase => {
        ...
        return (
          <div key={phase.phaseId} className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
            ...
            {/* Phase Accordion Header Bar */}
            <div 
              onClick={() => togglePhase(phase.phaseId)}
              className="p-4 sm:p-5 bg-slate-100/90 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/60 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition hover:bg-slate-200/60 dark:hover:bg-slate-800"
            >
              <div className="flex items-start sm:items-center space-x-2.5">
                <button className="p-1 rounded-lg text-slate-400 mt-0.5 sm:mt-0">
              ...
              <div className="flex items-center justify-between sm:justify-end space-x-3 pt-2 sm:pt-0 border-t sm:border-0 border-slate-200/60 dark:border-slate-700/40">
                <div className="text-left sm:text-right">
                  <span className="text-[11px] text-slate-400 font-medium">Phase Progress</span>
                  <div className="text-xs sm:text-sm font-black text-cyan-600 dark:text-cyan-400 font-mono">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
                        <div>
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            {subject.weightage > 0 && (
                              <span className="px-2 py-0.5 rounded text-[10px] sm:text-xs font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            {is100Done ? (
                              <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-500/20 text-emerald-500 border border-emerald-500/30">
                                <CheckCircle2 className="w-3.5 h-3.5" />
                                <span>100% Mastered (PYQs & Test Complete)</span>
                              </span>
                            ) : (
                              <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                        {subject.subTasks.map(task => {
                          ...
                          return (
                            <div
                              key={task.key}
                              className={`p-3 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-2 transition-all duration-150 ${
                                isChecked
                            ...
                            >
                              ...
                              >
                                <input
                                  ...
                                  checked={isChecked}
                                  onChange={() => {}}
                                  className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 pointer-events-none flex-shrink-0 mt-0.5"
                                />
                                <div className="flex items-center space-x-1.5 min-w-0">
                                  <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isChecked ? 'text-emerald-500' : 'text-slate-400'}`} />
                                  <span className={`text-xs font-bold leading-tight ${isChecked ? 'line-through opacity-80' : 'text-slate-900 dark:text-white'}`}>
                                    {task.label}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center justify-end sm:justify-start flex-shrink-0">
                                {isTest ? (
                                  <a
                                    ...
                                    className="px-2.5 py-1.5 rounded-lg text-[11px] font-extrabold bg-amber-500/15 dark:bg-amber-500/25 text-amber-700 dark:text-amber-300 hover:bg-amber-500 hover:text-white border border-amber-500/30 transition flex items-center gap-1 shadow-sm min-h-[34px]"
